...
const OrderReceipt: React.FC<OrderReceiptProps> = ({ order, fontFamily }) => {
  ...
  const fullDate = orderDate.toLocaleString('pt-BR', {
    day: '2-digit', month: '2-digit', year: 'numeric',
    hour: '2-digit', minute: '2-digit',
  });
  const est30 = new Date(orderDate.getTime() + 30 * 60000);
  const est50 = new Date(orderDate.getTime() + 50 * 60000);
  ...
  return (
    ...
    >
      ...
      {order.delivery_type === 'delivery' && (
        <p style={{ textAlign: 'center', fontSize: 10, fontWeight: 'bold' }}>
          🕐 Previsão: {estimateText}
        </p>
      )}
        ...
        <tbody>
          {(order.items || []).map((item, idx) => {
            ...
            <React.Fragment key={idx}>
              ...
              {item.observation && (
                <tr>
                  <td colSpan={2} style={{ padding: '0 0 1px 12px', fontSize: 10, fontStyle: 'italic', color: '#000' }}>
      ...
      {order.observation && (
        <p style={{ fontSize: 11, padding: '2px 0', fontStyle: 'italic' }}>
      ...
      {order.payment_method === 'dinheiro' && order.change_for && order.change_for > 0 && (
        <p style={{ fontSize: 11, padding: '1px 0' }}>
          <b>Troco para:</b> {formatPrice(order.change_for)}
      ...
      {order.address && (() => {
        const lastDash = order.address.lastIndexOf(' - ');
        const addr = lastDash > 0 ? order.address.slice(0, lastDash) : order.address;
        const bairro = lastDash > 0 ? order.address.slice(lastDash + 3) : null;
        return (
          <>
            <p style={{ fontSize: 11, padding: '1px 0' }}><b>Endereço:</b> {addr}</p>
            {bairro && <p style={{ fontSize: 11, padding: '1px 0' }}><b>Bairro:</b> {bairro}</p>}
          </>
        );
      })()}
...
export const getReceiptHTML = (order: Order): string => {
  ...
  const itemsHtml = (order.items || []).map(item => {
    ...
    const obsRow = item.observation ? `<tr><td colspan="2" style="padding:0 0 1px 12px;font-size:10px;font-style:italic;color:#000">Obs: ${item.observation}</td></tr>` : '';
    return mainRow + extrasRows + obsRow;
  }).join('');
...
${order.delivery_type === 'delivery' ? `<p class="center bold" style="font-size:10px">🕐 Previsão: ${estimateText}</p>` : ''}
<hr class="divider">
<table>${itemsHtml}</table>
${order.observation ? `<p class="info-row" style="font-style:italic;padding:2px 0"><b>Obs:</b> ${order.observation}</p>` : ''}
<hr class="divider">
<table>
...
${order.payment_method === 'dinheiro' && order.change_for && order.change_for > 0 ? `<p class="info-row"><b>Troco para:</b> ${formatPrice(order.change_for)}${order.change_for > order.total ? ` (troco: ${formatPrice(order.change_for - order.total)})` : ''}</p>` : ''}
<p class="info-row"><b>Entrega:</b> ${order.delivery_type === 'delivery' ? 'Delivery' : 'Retirada na loja'}</p>
${order.address ? (() => {
  const lastDash = order.address.lastIndexOf(' - ');
  const addr = lastDash > 0 ? order.address.slice(0, lastDash) : order.address;
  const bairro = lastDash > 0 ? order.address.slice(lastDash + 3) : null;
  return `<p class="info-row"><b>Endereço:</b> ${addr}</p>${bairro ? `<p class="info-row"><b>Bairro:</b> ${bairro}</p>` : ''}`;
})() : ''}
