...
const ProductDetailDialog = ({ item, open, onOpenChange }: ProductDetailDialogProps) => {
  ...
  const { addItem, globalAddons, setGlobalAddon } = useCart();
  ...
  useEffect(() => {
    if (open && item.id) {
      ...
      // Pre-populate addons from globalAddons
      setSelectedAddons(new Map(globalAddons));
  ...
  const isGlobalAddon = (addon: ProductAddon) => {
    const name = addon.name.toLowerCase();
    return name.includes('descartáv') || name.includes('descartav');
  };

  const changeAddonQty = (id: string, delta: number) => {
    const addon = addons.find(a => a.id === id);
    const maxQty = addon && isGlobalAddon(addon) ? 1 : 99;
    ...
    setSelectedAddons(prev => {
      ...
      // Persist global addons
      if (addon && isGlobalAddon(addon)) {
        setGlobalAddon(id, newVal);
      }
      ...
      <DialogContent className="max-w-4xl p-0 bg-card border-border overflow-hidden max-h-[90vh] flex flex-col md:flex-row md:items-stretch">
        ...
        <div className="md:w-1/2 flex flex-col overflow-y-auto">
          <div className="p-6 flex-1">
            ...
            {/* Observation */}
            <div className="mt-6">
              <span className="text-sm font-semibold text-foreground mb-2 block">Observação <span className="text-muted-foreground font-normal">(opcional)</span></span>
              <textarea
                value={observation}
                onChange={(e) => setObservation(e.target.value)}
                placeholder="Ex: Sem cebola, troco para R$50..."
                rows={2}
                className="w-full text-sm px-3 py-2 rounded-xl bg-background border border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary resize-none"
              />
            </div>
