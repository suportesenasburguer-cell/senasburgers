...
import PromoStepsDialog, { PromoStep, StepSelection } from './PromoStepsDialog';
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
...
const MenuSection = () => {
  ...
  const addPromoToCart = (promo: Promotion, selections?: StepSelection[]) => {
    const extras = selections?.map(s => s.product.name).join(', ');
    const desc = extras ? `${promo.description} — ${extras}` : promo.description;
    const firstDrink = selections?.[0];
    addItem({
      ...
      quantity: 1,
      addBatata: false,
      bebida: firstDrink ? { id: firstDrink.product.id, name: firstDrink.product.name, price: 0 } : null,
  ...
  const handleAddPromo = (promo: Promotion) => {
    if (promo.promo_steps && promo.promo_steps.length > 0) {
  ...
  // Group products by category_id, excluding hidden products
  const visibleProducts = products.filter((p) => !p.is_hidden);
  const groupedByCategory = categories
    .map((cat) => ({
      ...cat,
      items: visibleProducts.filter((p) => p.category_id === cat.id),
    }))
    ...
    <section id="menu" className="py-16 px-4">
      ...
      {pendingPromo && (
        <PromoStepsDialog
          ...
          onOpenChange={(open) => { setStepsDialogOpen(open); if (!open) setPendingPromo(null); }}
          steps={pendingPromo.promo_steps}
          onConfirm={(selections) => {
            addPromoToCart(pendingPromo, selections);
