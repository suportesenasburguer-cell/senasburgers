...
import PromoDrinkDialog from './PromoDrinkDialog';
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
...
const MenuSection = () => {
  ...
  const addPromoToCart = (promo: Promotion, drink?: { id: string; name: string }) => {
    const desc = drink ? `${promo.description} — ${drink.name}` : promo.description;
    addItem({
      ...
      quantity: 1,
      addBatata: false,
      bebida: drink ? { id: drink.id, name: drink.name, price: 0 } : null,
  ...
  const handleAddPromo = (promo: Promotion) => {
    if (promo.drink_groups && promo.drink_groups.length > 0) {
    ...
    <section id="menu" className="py-16 px-4">
      ...
      {pendingPromo && (
        <PromoDrinkDialog
          ...
          onOpenChange={(open) => { setDrinkDialogOpen(open); if (!open) setPendingPromo(null); }}
          drinkGroups={pendingPromo.drink_groups}
          onConfirm={({ product }) => {
            addPromoToCart(pendingPromo, { id: product.id, name: product.name });
