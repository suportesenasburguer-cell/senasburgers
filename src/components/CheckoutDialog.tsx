...
import { MessageCircle, CreditCard, Banknote, QrCode, MapPin, Store, Gift, X, User, Phone, Navigation, Ticket, Check, CalendarClock, Loader2 } from 'lucide-react';
import { isStoreOpen, getNextOpenDates, getAvailableTimeSlots, formatScheduledDateTime, DAY_LABELS } from '@/lib/store-hours';
...
// Neighborhoods are now fetched from DB
...
const CheckoutDialog = ({ open, onOpenChange, items, total, onConfirm }: CheckoutDialogProps) => {
  ...
  const [neighborhoods, setNeighborhoods] = useState<{ name: string; fee: number }[]>([]);
  const storeCurrentlyOpen = isStoreOpen();
  const [scheduledDate, setScheduledDate] = useState('');
  const [scheduledTime, setScheduledTime] = useState('');
  const nextOpenDates = getNextOpenDates(7);
  ...
  const needsScheduling = !storeCurrentlyOpen;
  const hasValidSchedule = !needsScheduling || (scheduledDate && scheduledTime);
  const isValid = paymentMethod && deliveryType && customerName.trim() && phoneDigits.length === 11 && hasValidNeighborhood && (deliveryType === 'pickup' || (street.trim() && houseNumber.trim() && neighborhood)) && hasValidSchedule;
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      // Build scheduled label
      let scheduledForLabel: string | undefined;
      if (needsScheduling && scheduledDate && scheduledTime) {
        const dateObj = nextOpenDates.find(d => d.toISOString().split('T')[0] === scheduledDate);
        if (dateObj) {
          scheduledForLabel = formatScheduledDateTime(dateObj, scheduledTime);
        }
      }
      ...
      await Promise.resolve(
        onConfirm({
          ...
          changeFor: paymentMethod === 'dinheiro' && changeFor && parseFloat(changeFor) > 0
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!isSubmitting) onOpenChange(o); }}>
      <DialogContent
        className="sm:max-w-md max-h-[90vh] overflow-y-auto bg-card border-border"
        onPointerDownOutside={(e) => { if (isSubmitting) e.preventDefault(); }}
        onEscapeKeyDown={(e) => { if (isSubmitting) e.preventDefault(); }}
      >
        <DialogHeader>
          <DialogTitle className="text-xl font-bold text-foreground">
            {needsScheduling ? '📅 Agendar Pedido' : 'Finalizar Pedido'}
          </DialogTitle>
        </DialogHeader>

        <div className="space-y-5 py-2">
          {/* Scheduling Section (when store is closed) */}
          {needsScheduling && (
            <div className="space-y-3 p-4 rounded-xl border-2 border-primary/30 bg-primary/5">
              ...
              <p className="text-xs text-muted-foreground">
                Estamos fechados agora. Escolha o dia e horário para receber seu pedido.
              </p>
              <div className="grid grid-cols-2 gap-2">
                ...
                <Select value={scheduledTime} onValueChange={setScheduledTime}>
                  ...
                  <SelectContent className="bg-popover border-border z-[200] max-h-48">
                    {timeSlots.map((t) => (
              ...
              {scheduledDate && scheduledTime && (
                <p className="text-xs text-primary font-medium">
                  ✅ Agendado para: {(() => {
                    const dateObj = nextOpenDates.find(d => d.toISOString().split('T')[0] === scheduledDate);
                    return dateObj ? formatScheduledDateTime(dateObj, scheduledTime) : '';
                  })()}
                </p>
              )}
            </div>
            ...
            {paymentMethod === 'dinheiro' && (
              <div className="mt-3 animate-fade-in">
                <Label className="text-xs text-muted-foreground mb-1.5 block">
                  Precisa de troco? Informe o valor da nota:
                </Label>
                <div className="flex items-center gap-2">
                  ...
                  {changeFor && parseFloat(changeFor) > finalTotal && (
                    <span className="text-xs text-muted-foreground">
                      Troco: {formatPrice(parseFloat(changeFor) - finalTotal)}
                    </span>
                  )}
                </div>
                {changeFor && parseFloat(changeFor) > 0 && parseFloat(changeFor) < finalTotal && (
                  <p className="text-xs text-destructive mt-1">
                    O valor deve ser maior que o total ({formatPrice(finalTotal)})
                  </p>
                )}
              </div>
            )}
          ...
          <Button
            ...
            className="w-full gradient-burger text-primary-foreground py-6 rounded-xl font-bold text-lg gap-2 disabled:opacity-90"
          >
            {isSubmitting ? (
              <>
              ...
              <>
                ...
                {needsScheduling ? '📅 Agendar pelo WhatsApp' : 'Enviar pelo WhatsApp'}
              </>
            )}
          </Button>
          {isSubmitting && (
            <p className="text-center text-xs text-muted-foreground -mt-2">
              Aguarde, estamos registrando seu pedido. Não feche esta janela.
            </p>
          )}
        </div>
      </DialogContent>
