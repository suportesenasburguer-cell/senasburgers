...
import { MessageCircle, CreditCard, Banknote, QrCode, MapPin, Store, Gift, X, User, Phone, Navigation, Ticket, Check, CalendarClock } from 'lucide-react';
import { getNextOpenDates, getAvailableTimeSlots, formatScheduledDateTime, DAY_LABELS } from '@/lib/store-hours';
...
const CheckoutDialog = ({ open, onOpenChange, items, total, onConfirm }: CheckoutDialogProps) => {
  ...
  const { isOpen: storeCurrentlyOpen } = useStoreStatus();
  const [scheduledDate, setScheduledDate] = useState('');
  const [scheduledTime, setScheduledTime] = useState('');
  const nextOpenDates = getNextOpenDates(7);
  const selectedDateObj = scheduledDate ? nextOpenDates.find(d => d.toISOString().split('T')[0] === scheduledDate) : null;
  const timeSlots = getAvailableTimeSlots(selectedDateObj ? selectedDateObj.getDay() : undefined);
  useEffect(() => {
      ...
      <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto bg-card border-border">
        ...
        <div className="space-y-5 py-2">
          ...
          >
            ...
            {isSubmitting ? 'Enviando...' : needsScheduling ? '📅 Agendar pelo WhatsApp' : 'Enviar pelo WhatsApp'}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
