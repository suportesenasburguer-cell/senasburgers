...
import {
  ...
  GripVertical, Ticket, Eye, EyeOff, ArrowUp, ArrowDown
} from 'lucide-react';
...
const Admin = () => {
  ...
  const [promoForm, setPromoForm] = useState({
    title: '', description: '', original_price: '', promo_price: '', valid_until: 'Hoje', items: '', image_url: '', is_active: true, promo_steps: [] as PromoStep[]
  });
  ...
  const handleAddonSubmit = async () => {
    if (!addonForm.name || addonForm.price === '' || addonForm.category_ids.length === 0) {
      toast({ title: 'Preencha nome, preço e pelo menos uma categoria', variant: 'destructive' });
      return;
    }
  ...
  const handlePromoSubmit = async () => {
    ...
    const payload = {
      ...
      promo_steps: promoForm.promo_steps,
    };
  ...
  const editPromo = (p: Promotion) => {
    ...
    setPromoForm({
      ...
      promo_steps: p.promo_steps || [],
    });
    setShowPromoForm(true);
  };

  const resetPromoForm = () => {
    setPromoForm({ title: '', description: '', original_price: '', promo_price: '', valid_until: 'Hoje', items: '', image_url: '', is_active: true, promo_steps: [] });
  ...
  const filteredAddons = addonCategoryFilter === 'all'
    ? addons
    : addons.filter(a => (a.category_ids?.length ? a.category_ids : [a.category_id]).includes(addonCategoryFilter));
      ...
      <div className="min-h-screen flex w-full bg-background">
        ...
        <main className="flex-1 min-h-screen">
          ...
          <header className="sticky top-0 z-40 glass border-b border-border h-14 flex items-center px-4 gap-3">
            ...
            <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">
              ...
              {activeTab === 'neighborhoods' && 'Bairros / Entregas'}
              {activeTab === 'users' && 'Usuários'}
              {activeTab === 'drivers' && 'Entregadores'}
            </h2>
          </header>
                          ...
                          <div className="flex gap-1 pr-3">
                            <Button
                              ...
                              onClick={async () => {
                                ...
                                const { error } = await (supabase as any).from('products').update({ is_hidden: newHidden }).eq('id', p.id);
                                if (error) { toast({ title: 'Erro', description: error.message, variant: 'destructive' }); return; }
                                toast({ title: newHidden ? 'Produto oculto!' : 'Produto visível!' });
                                fetchData();
                              }}
                              className={p.is_hidden ? 'text-muted-foreground hover:text-foreground' : 'text-foreground hover:text-muted-foreground'}
                              title={p.is_hidden ? 'Tornar visível' : 'Ocultar da vitrine'}
                            >
        ...
        {activeTab === 'addons' && (
          <div>
            ...
            {showAddonForm && (
              <div className="bg-card border border-border rounded-xl p-6 mb-6 animate-fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="text-sm text-muted-foreground mb-1 block">Categorias (aplica a todos da categoria)</label>
                    <div className="space-y-2 max-h-48 overflow-y-auto border border-input rounded-md p-3 bg-background">
                      {categories.map(c => (
                        <label key={c.id} className="flex items-center gap-2 cursor-pointer hover:bg-muted/50 rounded px-2 py-1.5 transition-colors">
                          <input
                            type="checkbox"
                            checked={addonForm.category_ids.includes(c.id)}
                            onChange={e => {
                              setAddonForm(f => ({
                                ...f,
                          ...
                          <span className="text-xs text-muted-foreground">({products.filter(p => p.category_id === c.id).length} produtos)</span>
                        </label>
                      ))}
                    </div>
                    <p className="text-xs text-muted-foreground mt-1">
                      O adicional ficará disponível para todos os produtos das categorias selecionadas
                    </p>
                  </div>
                  <div>
                    <label className="text-sm text-muted-foreground mb-1 block">Seção (opcional)</label>
                    <Input
                      value={addonForm.section}
                      onChange={e => setAddonForm(f => ({ ...f, section: e.target.value }))}
                      placeholder="Ex: Molhos, Extras, Toppings..."
                      list="addon-sections-list"
                    />
                    <datalist id="addon-sections-list">
                      {[...new Set(addons.map(a => a.section).filter(Boolean))].map(s => (
                        <option key={s} value={s!} />
                      ))}
                    </datalist>
                    <p className="text-xs text-muted-foreground mt-1">
                      Agrupa os adicionais por seção no cardápio. Deixe vazio para a seção padrão "Adicione no seu pedido".
                    </p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <Input
                      type="number"
                      min="1"
                      value={addonForm.max_qty}
                      onChange={e => setAddonForm(f => ({ ...f, max_qty: e.target.value }))}
            ...
            {(() => {
              const sectionNames = [...new Set(addons.map(a => a.section).filter(Boolean))] as string[];
              // Sort sections by the minimum sort_order of their addons
              sectionNames.sort((a, b) => {
                const minA = Math.min(...addons.filter(ad => ad.section === a).map(ad => ad.sort_order));
                const minB = Math.min(...addons.filter(ad => ad.section === b).map(ad => ad.sort_order));
                return minA - minB;
              });
              // Add default section
              const hasDefault = addons.some(a => !a.section);
              const allSections = hasDefault ? ['__default__', ...sectionNames] : sectionNames;
              ...
              const moveSection = async (index: number, direction: -1 | 1) => {
                ...
                // Batch update: assign sort_order based on section position
                const updates: Promise<any>[] = [];
                let sortCounter = 0;
                for (const sec of reordered) {
                  const sectionAddons = addons.filter(a =>
                    sec === '__default__' ? !a.section : a.section === sec
                  );
                  for (const addon of sectionAddons) {
                    updates.push(
                      (supabase as any).from('product_addons').update({ sort_order: sortCounter }).eq('id', addon.id)
                    );
                    sortCounter++;
                  }
                }
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              ...
              {categories.map(c => (
                ...
                >
                  {c.name} ({addons.filter(a => (a.category_ids?.length ? a.category_ids : [a.category_id]).includes(c.id)).length})
                </Button>
              ))}
            </div>
              ...
              <div className="bg-card border border-border rounded-xl p-6 mb-6 animate-fade-in">
                ...
                <div className="mt-6 border-t border-border pt-4">
                  <div className="flex items-center justify-between mb-3">
                    <label className="text-sm font-medium text-foreground">Etapas de escolha na promoção</label>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setPromoForm(f => ({ ...f, promo_steps: [...f.promo_steps, { title: '', groups: [{ label: '', product_ids: [] }] }] }))}
                    >
                      <Plus className="w-3 h-3 mr-1" /> Adicionar etapa
                    </Button>
                  </div>
                  {promoForm.promo_steps.length > 0 && (
                    <p className="text-xs text-muted-foreground mb-3">
                      Cada etapa aparece como um popup sequencial. Ex: Etapa 1 = Bebida, Etapa 2 = Molho.
                    </p>
                  )}
                  <div className="space-y-6">
                    {promoForm.promo_steps.map((step, si) => (
                      <div key={si} className="border border-border rounded-xl p-4 space-y-4 bg-card">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-bold text-primary bg-primary/10 px-2 py-0.5 rounded-full">Etapa {si + 1}</span>
                          <Input
                            value={step.title}
                            onChange={e => {
                              const updated = [...promoForm.promo_steps];
                              updated[si] = { ...updated[si], title: e.target.value };
                              setPromoForm(f => ({ ...f, promo_steps: updated }));
                            }}
                            placeholder="Título da etapa (ex: Escolha sua bebida)"
                            className="flex-1"
                          />
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => {
                              const updated = promoForm.promo_steps.filter((_, i) => i !== si);
                              setPromoForm(f => ({ ...f, promo_steps: updated }));
                            }}
                        ...
                        <div className="space-y-3 pl-4 border-l-2 border-primary/20">
                          {step.groups.map((group, gi) => (
                            <div key={gi} className="bg-muted/50 border border-border rounded-lg p-3 space-y-2">
                              <div className="flex items-center gap-2">
                                <Input
                                  value={group.label}
                                  onChange={e => {
                                    const updatedSteps = [...promoForm.promo_steps];
                                    const updatedGroups = [...updatedSteps[si].groups];
                                    updatedGroups[gi] = { ...updatedGroups[gi], label: e.target.value };
                                    updatedSteps[si] = { ...updatedSteps[si], groups: updatedGroups };
                                    setPromoForm(f => ({ ...f, promo_steps: updatedSteps }));
                                  }}
                                  placeholder="Nome do grupo (ex: Suco, Refrigerante)"
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    const updatedSteps = [...promoForm.promo_steps];
                                    updatedSteps[si] = { ...updatedSteps[si], groups: updatedSteps[si].groups.filter((_, i) => i !== gi) };
                                    setPromoForm(f => ({ ...f, promo_steps: updatedSteps }));
                                  }}
                                >
                                  <X className="w-3 h-3" />
                              ...
                              <div className="grid grid-cols-2 gap-1 max-h-40 overflow-y-auto">
                                {products.filter(p => p.is_active).map(prod => (
                                  <label key={prod.id} className="flex items-center gap-2 text-sm p-1.5 rounded hover:bg-muted cursor-pointer">
                                    <input
                                      type="checkbox"
                                      checked={group.product_ids.includes(prod.id)}
                                      onChange={e => {
                                        const updatedSteps = [...promoForm.promo_steps];
                                        const updatedGroups = [...updatedSteps[si].groups];
                                        if (e.target.checked) {
                                          updatedGroups[gi] = { ...updatedGroups[gi], product_ids: [...updatedGroups[gi].product_ids, prod.id] };
                                        } else {
                                          updatedGroups[gi] = { ...updatedGroups[gi], product_ids: updatedGroups[gi].product_ids.filter(id => id !== prod.id) };
                                        }
                                        updatedSteps[si] = { ...updatedSteps[si], groups: updatedGroups };
                                        setPromoForm(f => ({ ...f, promo_steps: updatedSteps }));
                                      }}
                          ...
                          <Button
                            ...
                            onClick={() => {
                              const updatedSteps = [...promoForm.promo_steps];
                              updatedSteps[si] = { ...updatedSteps[si], groups: [...updatedSteps[si].groups, { label: '', product_ids: [] }] };
                              setPromoForm(f => ({ ...f, promo_steps: updatedSteps }));
                            }}
                          >
