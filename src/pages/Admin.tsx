...
import {
  ...
  GripVertical, Ticket, Eye, EyeOff
} from 'lucide-react';
...
const Admin = () => {
  ...
  const [addonForm, setAddonForm] = useState({ name: '', price: '', category_ids: [] as string[], section: '' });
  ...
  const [promoForm, setPromoForm] = useState({
    title: '', description: '', original_price: '', promo_price: '', valid_until: 'Hoje', items: '', image_url: '', is_active: true, drink_groups: [] as DrinkGroup[]
  });
  ...
  const editAddon = (a: Addon) => {
    ...
    setAddonForm({
      ...
      category_ids: a.category_ids?.length ? a.category_ids : (a.category_id ? [a.category_id] : []),
      section: a.section || '',
    });
  ...
  const resetAddonForm = () => {
    setAddonForm({ name: '', price: '', category_ids: [], section: '' });
  ...
  const handlePromoSubmit = async () => {
    ...
    const payload = {
      ...
      drink_groups: promoForm.drink_groups,
    };
  ...
  const editPromo = (p: Promotion) => {
    ...
    setPromoForm({
      ...
      drink_groups: p.drink_groups || [],
    });
    setShowPromoForm(true);
  };

  const resetPromoForm = () => {
    setPromoForm({ title: '', description: '', original_price: '', promo_price: '', valid_until: 'Hoje', items: '', image_url: '', is_active: true, drink_groups: [] });
              ...
              <div className="bg-card border border-border rounded-xl p-6 mb-6 animate-fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Agrupa os adicionais por seção no cardápio. Deixe vazio para a seção padrão.
                    </p>
                  </div>
                </div>
                          ...
                          <div className="flex items-center gap-3 flex-1">
                            ...
                            <div>
                              ...
                              <span className="text-sm text-muted-foreground">
                                Categorias: {(a.category_ids?.length ? a.category_ids : [a.category_id]).map(id => getCategoryName(id)).filter(Boolean).join(', ')}{a.section ? ` · Seção: ${a.section}` : ''} · Aplica a {products.filter(p => (a.category_ids?.length ? a.category_ids : [a.category_id]).includes(p.category_id)).length} produtos
                              </span>
                            </div>
                          </div>
                          <div className="flex gap-1">
              ...
              <div className="bg-card border border-border rounded-xl p-6 mb-6 animate-fade-in">
                ...
                <div className="mt-6 border-t border-border pt-4">
                  <div className="flex items-center justify-between mb-3">
                    <label className="text-sm font-medium text-foreground">Escolha de bebida na promoção</label>
                    <button
                      onClick={() => {
                        if (promoForm.drink_groups.length === 0) {
                          setPromoForm(f => ({ ...f, drink_groups: [{ label: '', product_ids: [] }] }));
                        } else {
                          setPromoForm(f => ({ ...f, drink_groups: [] }));
                        }
                      }}
                      className={cn(
                        'flex items-center gap-2 px-3 py-1.5 rounded-lg border text-sm transition-colors',
                        promoForm.drink_groups.length > 0
                          ? 'border-primary bg-primary/10 text-primary'
                          : 'border-border text-muted-foreground hover:border-primary/50'
                      )}
                    >
                      {promoForm.drink_groups.length > 0 ? '✅ Ativado' : 'Ativar'}
                    </button>
                  </div>
                  {promoForm.drink_groups.length > 0 && (
                    <div className="space-y-4">
                      <p className="text-xs text-muted-foreground">Configure os grupos de bebidas. O cliente escolherá um grupo e depois o produto.</p>
                      {promoForm.drink_groups.map((group, gi) => (
                        <div key={gi} className="bg-muted/50 border border-border rounded-lg p-4 space-y-3">
                          <div className="flex items-center gap-2">
                            <Input
                              value={group.label}
                              onChange={e => {
                                const updated = [...promoForm.drink_groups];
                                updated[gi] = { ...updated[gi], label: e.target.value };
                                setPromoForm(f => ({ ...f, drink_groups: updated }));
                              }}
                            ...
                            <Button
                              ...
                              size="icon"
                              onClick={() => {
                                const updated = promoForm.drink_groups.filter((_, i) => i !== gi);
                                setPromoForm(f => ({ ...f, drink_groups: updated }));
                              }}
                          ...
                          <div>
                            <label className="text-xs text-muted-foreground mb-1 block">Produtos deste grupo:</label>
                                ...
                                <label key={prod.id} className="flex items-center gap-2 text-sm p-1.5 rounded hover:bg-muted cursor-pointer">
                                  <input
                                    ...
                                    onChange={e => {
                                      const updated = [...promoForm.drink_groups];
                                      if (e.target.checked) {
                                        updated[gi] = { ...updated[gi], product_ids: [...updated[gi].product_ids, prod.id] };
                                      } else {
                                        updated[gi] = { ...updated[gi], product_ids: updated[gi].product_ids.filter(id => id !== prod.id) };
                                      }
                                      setPromoForm(f => ({ ...f, drink_groups: updated }));
                                    }}
                                    className="rounded"
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => setPromoForm(f => ({ ...f, drink_groups: [...f.drink_groups, { label: '', product_ids: [] }] }))}
                      >
