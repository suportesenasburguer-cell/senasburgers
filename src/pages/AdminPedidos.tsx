...
import { ArrowLeft, Clock, ChefHat, Truck, CheckCircle2, XCircle, RefreshCw, Printer, Volume2, Eye, EyeOff, MessageCircle, CalendarClock } from 'lucide-react';
...
const AdminPedidos = () => {
  ...
  useEffect(() => {
    ...
    const fetchDrivers = async () => {
      ...
      // Use admin_list_users RPC to get names/emails
      const { data: allUsers } = await (supabase as any).rpc('admin_list_users');

      const driverList: Driver[] = driverIds.map((id: string) => {
        const u = (allUsers || []).find((u: any) => u.id === id);
        return {
          user_id: id,
          name: u?.display_name || u?.email || id.slice(0, 8),
        };
      });
  ...
  const advanceStatus = async (order: Order) => {
    ...
    // When moving to "delivering", assign the selected driver
    if (nextStatus === 'delivering' && order.delivery_type === 'delivery') {
      const driverId = selectedDrivers[order.id];
      if (!driverId) {
        toast({ title: 'Selecione um entregador', description: 'Escolha o entregador antes de enviar para entrega.', variant: 'destructive' });
        return;
      }
              ...
              <div key={order.id} className="bg-card border border-border rounded-xl p-5">
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div>
                    ...
                    {order.scheduled_for && (
                      <span className="inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full bg-primary/15 text-primary border border-primary/30 mt-1">
                        <CalendarClock className="w-3 h-3" />
                        📅 Agendado: {order.scheduled_for}
                      </span>
                    )}
                  </div>
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground mb-3">
                  ...
                  {order.delivered_at && (
                    <span className="text-emerald-400 font-medium">
                      ✅ Entregue pelo entregador em {formatDate(order.delivered_at)}
                    </span>
                  )}
                  {order.driver_id && !order.delivered_at && (
                ...
                {/* Driver selector - show when order is preparing and delivery type */}
                {order.status === 'preparing' && order.delivery_type === 'delivery' && drivers.length > 0 && (
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-xs text-muted-foreground whitespace-nowrap">🛵 Entregador:</span>
                    <Select
                      value={selectedDrivers[order.id] || ''}
                      onValueChange={(value) => setSelectedDrivers(prev => ({ ...prev, [order.id]: value }))}
                    >
