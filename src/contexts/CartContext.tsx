...
interface CartContextType {
  ...
  globalAddons: Map<string, number>;
  setGlobalAddon: (addonId: string, qty: number) => void;
}
...
export const CartProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [globalAddons, setGlobalAddons] = useState<Map<string, number>>(new Map());
  ...
  const setGlobalAddon = (addonId: string, qty: number) => {
    setGlobalAddons(prev => {
      const next = new Map(prev);
      if (qty <= 0) next.delete(addonId);
      else next.set(addonId, qty);
      return next;
    });
  };
